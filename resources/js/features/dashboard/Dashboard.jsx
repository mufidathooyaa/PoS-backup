...
export function Dashboard() {
    ...
    const loadDashboard = useCallback(async () => {
        ...
        try {
            ...
            const outletParam = activeOutlet?.id
            ...
            setTrend(
                trendRes.map((res, i) => ({
                    day: new Date(trendDays[i]).toLocaleDateString("id-ID", {
                        weekday: "short",
                    }),
    ...
    const penjualanHariIni = today.ringkasan.penjualan_bersih_setelah_refund;
    const penjualanKemarin =
        yesterday.ringkasan.penjualan_bersih_setelah_refund;
    const persenPerubahan =
        penjualanKemarin > 0
            ? (
                  ((penjualanHariIni - penjualanKemarin) / penjualanKemarin) *
                  100
              ).toFixed(1)
            : null;

    const stokRendah = catalog.filter(
        (p) => p.track_stock && p.stok_tersedia <= p.stok_minimum,
    );
    const kasirAktif = openShifts.filter(
        (s) => s.user?.role?.nama_peran === "Kasir",
    ).length;
    const operatorAktif = openShifts.filter(
        (s) => s.user?.role?.nama_peran === "Operator Inventaris",
    ).length;
        ...
        <>
                ...
                actions={
                    <div className="flex gap-2">
                        {pendingShiftReviews > 0 && (
                            ...
                            >
                                <AlertTriangle size={16} /> Tinjau Selisih Shift
                                ({pendingShiftReviews})
                            </button>
                        )}
                        {pendingApproval > 0 && (
                            <button
                                ...
                                onClick={() =>
                                    navigate("/inventaris", {
                                        state: { tab: "Penyesuaian Stok" },
                                    })
                                }
                            >
                                <ClipboardCheck size={16} /> Setujui Stok (
                                {pendingApproval})
                            </button>
                        )}
                    </div>
            ...
            <div className="grid grid-cols-4 gap-3">
                <Kpi
                    ...
                    value={formatIDR(penjualanHariIni)}
                    note={
                        persenPerubahan !== null
                            ? `${persenPerubahan >= 0 ? "↑" : "↓"} ${Math.abs(persenPerubahan)}% dari kemarin`
                ...
                <Kpi
                    icon={ReceiptText}
                    label="Transaksi"
                    value={today.ringkasan.jumlah_transaksi}
                    note={
                        today.ringkasan.jumlah_transaksi > 0
                            ? `Rata-rata ${formatIDR(penjualanHariIni / today.ringkasan.jumlah_transaksi)}`
                            : "Belum ada transaksi"
                    }
                    tone="blue"
                />
                <Kpi
                    ...
                    note={`${stokRendah.length} produk perlu segera dipesan`}
                    tone="amber"
                />
                <Kpi
                    icon={Clock3}
                    ...
                    note={`${kasirAktif} kasir • ${operatorAktif} operator`}
                    tone="emerald"
                />
            </div>
            ...
            <div className="mt-3 grid grid-cols-12 gap-3">
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="mt-1 text-xs text-slate-500">
                                Total penjualan kotor per hari
                            </p>
                        </div>
                    </div>
                ...
                <div className="card col-span-4 p-4">
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                        Berdasarkan jumlah terjual hari ini
                    </p>
                    <div className="mt-4 space-y-4">
                        {today.produk_terlaris.length === 0 && (
                        ...
                        {today.produk_terlaris.slice(0, 4).map((p) => {
                            const max =
                                today.produk_terlaris[0]?.total_terjual || 1;
                            return (
